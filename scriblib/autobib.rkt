...
(define-syntax (auto-bib-accessor-maker stx)
  (syntax-case stx ()
    [(_ field)
     (with-syntax ([id (format-id #'field
                                  #:source #'field
                                  "auto-bib-~a"
                                  (syntax-e #'field))])
...
(auto-bib-accessor-maker title)
(auto-bib-accessor-maker location)
(auto-bib-accessor-maker url)
(auto-bib-accessor-maker note)
(auto-bib-accessor-maker is-book?)
(auto-bib-accessor-maker key)
(auto-bib-accessor-maker specific)
(define (make-auto-bib . args)
  (apply auto-bib args)) 
(define (auto-bib? v)
  (and (hash-eq? v)
       (hash-ref v '__auto-bib #f)))

;(define-struct auto-bib (author date title location url note is-book? key specific))
(define auto-bib
  (match-lambda*
    [(list author date title location url note is-book? key specific)
     ;; The original structure had the above fields. For compatability, our
     ;; hash table should have them as well, as well as accessors.
     (define h (make-hasheq))
     (hash-set! h '__auto-bib #t)
     (hash-set! h 'author author)
     (hash-set! h 'date date)
     (hash-set! h 'title title)
     (hash-set! h 'location location)
     (hash-set! h 'url url)
     (hash-set! h 'note note)
     (hash-set! h 'is-book? is-book?)
     (hash-set! h 'key key)
     (hash-set! h 'specific specific)
     h]
    [(list h)
     h]))
